...
const DOMElements = {
    ...
    passOddsContainer: document.querySelector('.md\\:col-span-2.space-y-4 > .grid.grid-cols-2'),
...
function updateUI() {
    ...
    document.querySelectorAll('.betting-spot').forEach(spot => {
        ...
        container.classList.toggle('ring-4', point && `place${point}` === betKey);
        container.classList.toggle('ring-offset-2', point && `place${point}` === betKey);
        container.classList.toggle('ring-offset-craps-green', point && `place${point}` === betKey);
        container.classList.toggle('ring-white', point && `place${point}` === betKey);
        container.classList.toggle('shadow-lg', point && `place${point}` === betKey);
    ...
    rollHistory.slice(-25).forEach(item => {
    ...
    DOMElements.chipContainer.querySelectorAll('button').forEach(btn => {
        btn.classList.toggle('ring-4', parseInt(btn.dataset.value) === selectedChip);
        btn.classList.toggle('ring-craps-gold', parseInt(btn.dataset.value) === selectedChip);
    });
}
...
function calculateProbabilities() {
    ...
    for (let total = 2; total <= 12; total++) {
        ...
        if (gameState === 'COME_OUT') {
            ...
            } else if (arePlaceBetsWorking && bets[`place${total}`]) {
                netWinLoss += (bets[`place${total}`] || 0) * PLACE_PAYOUTS[total].n / PLACE_PAYOUTS[total].d;
            }
        }
...
function handleRoll() {
    ...
    setTimeout(() => {
        ...
        let winAmount = 0, lossAmount = 0, betsToReturnOnWin = 0, roundEnded = false;
        let message = '', messageType = 'roll';
        ...
        if (gameState === 'COME_OUT') {
            ...
            if (total === 7) {
                ...
                message = 'Winner 7!';
                messageType = 'win';
                bankroll += winAmount + betsToReturnOnWin;
                delete bets.passLine;
                setLastResult(message, winAmount, 'win');
            } else {
                point = total;
                gameState = 'POINT_ON';
                ...
                if (bets[placeBetOnPointKey]) {
                    const amount = bets[placeBetOnPointKey];
                    delete bets[placeBetOnPointKey];
                    const priority = [6, 8, 5, 9, 4, 10, 3, 11, 2, 12];
                    let moved = false;
                    for (const num of priority) {
                        if (num !== point && !bets[`place${num}`]) {
                            bets[`place${num}`] = amount;
                            message += `. Bet moved to ${num}.`;
                            moved = true;
                            break;
                        }
                    }
                ...
                setLastResult(message, 0, 'info');
            }
        } else { // POINT_ON
            if (total === point) {
                winAmount += bets.passLine || 0;
                if (bets.odds && point) winAmount += (bets.odds || 0) * ODDS_PAYOUTS[point].n / ODDS_PAYOUTS[point].d;
                if (arePlaceBetsWorking && bets[`place${total}`]) {
                    winAmount += bets[`place${total}`] * PLACE_PAYOUTS[total].n / PLACE_PAYOUTS[total].d;
                }
                betsToReturnOnWin = Object.values(bets).reduce((s, v) => s + (v || 0), 0);
                message = 'Point Hit!'; roundEnded = true; messageType = 'win';
            } else if (total === 7) {
                lossAmount = (bets.passLine || 0) + (bets.odds || 0);
                let returnedBets = 0;
                PLACE_NUMBERS.forEach(num => {
                    if (arePlaceBetsWorking) lossAmount += bets[`place${num}`] || 0;
                    else returnedBets += bets[`place${num}`] || 0;
                });
                bankroll += returnedBets;
                message = 'Seven Out'; roundEnded = true; messageType = 'loss';
        ...
        if (roundEnded) {
            if (winAmount > 0) {
                ...
                setLastResult(message, lossAmount, 'loss');
            }
            gameState = 'COME_OUT';
            point = null;
            arePlaceBetsWorking = false;
            ...
            Object.keys(bets).forEach(key => {
                if (key.startsWith('place')) newBets[key] = bets[key];
            });
            bets = newBets;
        } else if (message && winAmount === 0 && gameState === 'COME_OUT') {
            setLastResult(message, 0, 'info');
        } else if (!message) {
...
function initialize() {
    ...
    DOMElements.passOddsContainer.innerHTML = ''; // Clear placeholder
    ...
    document.querySelectorAll('.betting-spot').forEach(spot => {
        spot.addEventListener('click', () => handleBet(spot.dataset.betKey));
        spot.addEventListener('contextmenu', (e) => { e.preventDefault(); removeBet(spot.dataset.betKey); });
    });
    ...
    DOMElements.restartButton.onclick = () => {
        DOMElements.bankrollInput.value = bankroll;
